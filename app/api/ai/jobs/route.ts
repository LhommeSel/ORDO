  ...
  try {
    ...
    const instructions = [
      ...
      parsed.job.kind === 'diplomacy' && typeof parsed.job.domainContext.respondingCountryId === 'string' && !isDiplomaticMeeting
        ...
        : '',
      isDiplomaticMeeting
        ? 'Pour une rencontre diplomatique, diplomaticMove.scope doit être general_dialogue. Réponds comme une synthèse de la position finale de tous les participants listés dans le contexte, et remplis participantResponses avec une entrée distincte pour chaque participant (participantId exact, kind, position, acceptedTerms, rejectedTerms, conditionalTerms, rationale). publicMessage peut résumer ces positions, mais ne fabrique jamais de consensus si un participant oppose une ligne rouge ou si une réponse manque. privateDecision.actorId doit reprendre le premier respondingCountryId fourni par le moteur. kind=accept signifie que tous les participants acceptent le projet final ; kind=counter signifie qu’au moins un participant demande un ajustement ; kind=refuse signifie que le projet est rejeté. Le domainContext peut fournir dossierContext et negotiationHistory : utilise-les pour reprendre les engagements déjà discutés, les positions individuelles, les conditions ouvertes et les réponses des rencontres précédentes ; ne repars jamais d’une page blanche et ne transforme pas un point ouvert en accord implicite.'
        : '',
      parsed.job.kind === 'diplomacy'
